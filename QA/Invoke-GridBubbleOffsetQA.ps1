...
function Report-StaleRequests {
    ...
    foreach ($request in @(Get-ChildItem -LiteralPath $runsRoot -Filter 'grid-bubble-offset.request.json' -File -Recurse -ErrorAction SilentlyContinue)) {
        try {
...
function Verify-Result([string]$Path) {
    ...
    if ($model -ine $request.modelPath -or $report.FixtureId -cne 'GridBubbleOffsetEmpty' -or $report.AssemblyIdentity.Sha256 -ine $request.assemblySha256) {
    ...
    if ((Get-Sha256 $model) -ine $report.FixtureSha256 -or
        (Get-Sha256 (Join-Path $repoRoot 'QA\Fixtures\GridBubbleOffsetEmpty.rvt')) -ine $report.FixtureSha256) {
...
$installedManifest = Join-Path $addinsRoot 'Repato.GridBubbleOffset.TestRunner.addin'
...
if ($Action -eq 'Install') {
    ...
    Copy-Item -LiteralPath (Join-Path $repoRoot 'QA\Repato.GridBubbleOffset.TestRunner.addin') -Destination $installedManifest -Force
...
if ((Get-Sha256 $installedManifest) -ine (Get-Sha256 (Join-Path $repoRoot 'QA\Repato.GridBubbleOffset.TestRunner.addin'))) { throw 'Installed QA manifest differs from source.' }
$fixture = Assert-ChildPath (Join-Path $repoRoot 'QA\Fixtures\GridBubbleOffsetEmpty.rvt') (Join-Path $repoRoot 'QA\Fixtures')
$provenance = Get-Content -LiteralPath (Join-Path $repoRoot 'QA\Fixtures\GridBubbleOffsetEmpty.provenance.json') -Raw | ConvertFrom-Json
$fixtureHash = Get-Sha256 $fixture
$approvedGridNames = @('A','B','C','D','1','2','3','4')
if ($provenance.fixtureId -cne 'GridBubbleOffsetEmpty' -or $fixtureHash -ine $provenance.sourceSha256 -or
...
$runDirectory = Assert-ChildPath (Join-Path $runsRoot ('grid-bubble-offset-' + $requestId)) $runsRoot
New-Item -ItemType Directory -Path $runDirectory | Out-Null
$model = Join-Path $runDirectory 'model.rvt'
Copy-Item -LiteralPath $fixture -Destination $model
@{ fixtureId = 'GridBubbleOffsetEmpty'; sourceSha256 = $fixtureHash; requiredGridNames = $approvedGridNames } |
    ConvertTo-Json | Set-Content -LiteralPath ($model + '.fixture.json') -Encoding UTF8
$requestPath = Join-Path $runDirectory 'grid-bubble-offset.request.json'
...
$previousRequest = $env:REPATO_QA_GRID_BUBBLE_OFFSET_REQUEST
try {
    $env:REPATO_QA_GRID_BUBBLE_OFFSET_REQUEST = $requestPath
...
finally { $env:REPATO_QA_GRID_BUBBLE_OFFSET_REQUEST = $previousRequest }
