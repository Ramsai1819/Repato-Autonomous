...
Test("qa-exact", (_,_,u,s) => { foreach(string n in new[]{"Repato.CreateLevels.TestRunner.addin","Repato.CreateGrids.TestRunner.addin","Repato.TestRunner.addin","Repato.GridBubbleVisibility.TestRunner.addin","Repato.GridBubbleOffset.TestRunner.addin","Repato.GridResequence.TestRunner.addin","Repato.WelcomeSmoke.TestRunner.addin"}) { File.WriteAllText(Path.Combine(s,n),"qa"); File.WriteAllText(Path.Combine(u,n),"qa"); } }, true);
