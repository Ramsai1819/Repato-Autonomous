...
Test("qa-exact", (_,_,u,s) => { foreach(string n in new[]{"Repato.CreateLevels.TestRunner.addin","Repato.TestRunner.addin"}) { File.WriteAllText(Path.Combine(s,n),"qa"); File.WriteAllText(Path.Combine(u,n),"qa"); } }, true);
