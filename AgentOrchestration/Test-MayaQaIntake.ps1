...
$ids = @('welcome-smoke','create-grids-world-axis-v1','create-levels','grid-bubble-visibility-v1','grid-bubble-offset-v1','grid-resequence-v1','create-plan-views-v1')
foreach ($id in $ids) {
...
$expected = if ($id -eq 'create-levels') { 'create-levels-elevations-v1' } elseif ($id -eq 'welcome-smoke') { 'welcome-supervised-dialog-v1' } else { $id }
if ($r.QaWorkflowId -ne $expected -or $r.SideEffectsPerformed -or $r.NextAllowedOperation -ne 'qa-run-plan') { throw "Valid intake mismatch: $id" }
}
$failed = $false; try { Invoke-Intake @('-Operation','qa-intake','-TaskId','task-test-01','-WorkflowId','workflow-test-01','-QaWorkflowId','create-schedules','-UserRequest','create-schedules','-DryRun') | Out-Null } catch { $failed = $true }; if (-not $failed) { throw 'Unsupported workflow accepted.' }
...
'Maya QA intake checks passed: 17'
