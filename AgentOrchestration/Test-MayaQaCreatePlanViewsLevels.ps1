...
if($runner.IndexOf('EnsureRequiredLevels', [StringComparison]::Ordinal) -lt 0 -or $runner.IndexOf('Execute(app, report)', [StringComparison]::Ordinal) -lt $runner.IndexOf('EnsureRequiredLevels', [StringComparison]::Ordinal)){throw 'Level preparation does not precede native execution.'}
