...
$wf=Get-DeployWorkflow $store $boot.TaskId $boot.WorkflowId;$data=Read-RepatoTaskStore $store;$task=Find-RepatoTask $data $boot.TaskId;$reset=Invoke-RepatoWorkflowMutation $store $boot.TaskId $boot.WorkflowId $wf.workflowRevision $task.revision {param($x)$x.qaBuildStatus='requested';return $x}
