...
public sealed class GridBubbleVisibilityQaApplication : IExternalApplication
{
    public const string RequestEnvironmentVariable = "REPATO_QA_GRID_BUBBLE_REQUEST";
    ...
    private void RunOnce(object? sender, IdlingEventArgs args)
    {
        ...
        string requestId = "";
        var report = new TestRunReport { TestId = GridBubbleVisibilityTestCommand.SupportedTestId, Inputs = new { } };
        try
        {
            ...
            if (testId != GridBubbleVisibilityTestCommand.SupportedTestId)
            ...
            if (!string.Equals(expectedAssembly, TestRunReport.Hash(typeof(GridBubbleVisibilityQaApplication).Assembly.Location), StringComparison.OrdinalIgnoreCase))
            ...
            string fixture = QAPathPolicy.ValidatePath(
                QAPathPolicy.RepositoryRoot + @"\QA\Fixtures\GridBubbleVisibilityEmpty.rvt",
            ...
            if (provenance.RootElement.GetProperty("fixtureId").GetString() != "GridBubbleVisibilityEmpty" ||
            ...
            report = GridBubbleVisibilityTestCommand.Run(app, testId);
        }
        catch (Exception ex)
        {
