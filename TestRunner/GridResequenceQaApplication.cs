...
public sealed class GridResequenceQaApplication : IExternalApplication
{
    public const string RequestEnvironmentVariable = "REPATO_QA_GRID_RESEQUENCE_REQUEST";
    ...
    private void RunOnce(object? sender, IdlingEventArgs args)
    {
        ...
        string requestId = "";
        var report = new TestRunReport { TestId = GridResequenceTestCommand.SupportedTestId, Inputs = new { } };
        try
        {
            ...
            if (testId != GridResequenceTestCommand.SupportedTestId)
            ...
            if (!string.Equals(expectedAssembly, TestRunReport.Hash(typeof(GridResequenceQaApplication).Assembly.Location), StringComparison.OrdinalIgnoreCase))
            ...
            string fixture = QAPathPolicy.ValidatePath(
                QAPathPolicy.RepositoryRoot + @"\QA\Fixtures\GridResequenceEmpty.rvt",
            ...
            if (provenance.RootElement.GetProperty("fixtureId").GetString() != "GridResequenceEmpty" ||
            ...
            report = GridResequenceTestCommand.Run(app, testId);
        }
        catch (Exception ex)
        {
        ...
        catch (Exception ex)
        {
            ...
            app.Application.WriteJournalComment("Repato Grid Resequence QA evidence failure: " + ex, false);
        }
    ...
    {
        try { File.AppendAllText((_requestPath ?? Path.Combine(QAPathPolicy.RunsRoot, "grid-resequence-startup")) + ".startup.log", DateTimeOffset.UtcNow.ToString("O") + " " + message + Environment.NewLine); } catch { }
    }
}
