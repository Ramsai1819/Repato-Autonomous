...
public sealed class WelcomeSmokeQaApplication : IExternalApplication
{
    public const string RequestEnvironmentVariable = "REPATO_QA_WELCOME_REQUEST";
    private string? _requestPath;

    public Result OnStartup(UIControlledApplication application)
    {
        _requestPath = Environment.GetEnvironmentVariable(RequestEnvironmentVariable);
        Log("add-in startup reached; request=" + (_requestPath ?? "<missing>"));
    ...
    private void RunOnce(object? sender, IdlingEventArgs args)
    {
        if (sender is not UIApplication app)
            return;
        app.Idling -= RunOnce;
        Log("request path detected; command execution starting");
        string? receiptPath = null;
        string requestId = "";
        var report = new TestRunReport { TestId = WelcomeSmokeTestCommand.SupportedTestId, Inputs = new { } };
        try
        {
            ...
            if (testId != WelcomeSmokeTestCommand.SupportedTestId)
            ...
            if (!string.Equals(expectedAssembly, TestRunReport.Hash(typeof(WelcomeSmokeQaApplication).Assembly.Location), StringComparison.OrdinalIgnoreCase))
            ...
            report = WelcomeSmokeTestCommand.Run(app, testId);
            Log("command execution completed; status=" + report.Status);
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            {
                QaRunnerFoundation.WriteReceiptAtomic(receiptPath,
                    new { RequestId = requestId, report.RunId, ReportPath = reportPath, report.Status });
                Log("result path written; receipt=" + receiptPath + "; report=" + reportPath);
            }
        }
        catch (Exception ex)
        {
            // The controller treats a missing receipt as failure, never a pass.
            app.Application.WriteJournalComment("Repato Welcome Smoke QA evidence failure: " + ex, false);
        }
    ...
    private void Log(string message)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(_requestPath)) return;
            string? directory = Path.GetDirectoryName(_requestPath);
            if (directory is null || !Directory.Exists(directory)) return;
            File.AppendAllText(Path.Combine(directory, "welcome.startup.log"),
