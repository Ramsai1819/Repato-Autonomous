...
public sealed class GridBubbleOffsetQaApplication : IExternalApplication
{
    public const string RequestEnvironmentVariable = "REPATO_QA_GRID_BUBBLE_OFFSET_REQUEST";
    private string? _requestPath;

    public Result OnStartup(UIControlledApplication application)
    {
        _requestPath = Environment.GetEnvironmentVariable(RequestEnvironmentVariable);
        Log("add-in startup reached; request=" + (_requestPath ?? "<none>"));
    ...
    private void RunOnce(object? sender, IdlingEventArgs args)
    {
        ...
        string requestId = "";
        var report = new TestRunReport { TestId = GridBubbleOffsetTestCommand.SupportedTestId, Inputs = new { } };
        try
        {
            ...
            Log("request path detected: " + requestPath);
            ...
            if (testId != GridBubbleOffsetTestCommand.SupportedTestId)
            ...
            if (!string.Equals(expectedAssembly, TestRunReport.Hash(typeof(GridBubbleOffsetQaApplication).Assembly.Location), StringComparison.OrdinalIgnoreCase))
            ...
            string fixture = QAPathPolicy.ValidatePath(
                QAPathPolicy.RepositoryRoot + @"\QA\Fixtures\GridBubbleOffsetEmpty.rvt",
            ...
            if (provenance.RootElement.GetProperty("fixtureId").GetString() != "GridBubbleOffsetEmpty" ||
            ...
            report = GridBubbleOffsetTestCommand.Run(app, testId);
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            if (receiptPath is not null)
            {
                ...
                Log("result path written: " + receiptPath);
            }
        }
        catch (Exception ex)
        {
            // The controller treats a missing receipt as failure, never a pass.
            app.Application.WriteJournalComment("Repato Grid Bubble Visibility QA evidence failure: " + ex, false);
        }
    ...
    {
        try { File.AppendAllText((_requestPath ?? Path.Combine(QAPathPolicy.RunsRoot, "grid-bubble-offset-startup")) + ".startup.log", DateTimeOffset.UtcNow.ToString("O") + " " + message + Environment.NewLine); } catch { }
    }
}
